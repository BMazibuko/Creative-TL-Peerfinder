...
from flask import Flask, request, jsonify, render_template, redirect, url_for, Response, flash, current_app
import pandas as pd
import boto3
...
app.config.update(
    ...
    MAIL_PASSWORD="arkwdwrsgitqbpdv",
)
mail = Mail(app)
...
s3 = boto3.client('s3')
CSV_OBJECT_KEY = 'ctl_peer-matcing_data.csv'
...
def download_csv():
    try:
        ...
        expected_columns = [
            ...
            'program', 'topic_module', 'learning_preferences', 'availability',
            'preferred_study_setup', 'kind_of_support', 'connection_type',
            'timestamp', 'matched', 'group_id', 'unpair_reason', 'matched_timestamp'
        ]
        for col in expected_columns:
            if col not in df.columns:
                df[col] = '' if col not in ['matched'] else False
        return df
...
def join_queue():
    ...
    required_fields = [name, phone, email, country, language, cohort, program,
    ...
    if connection_type == 'find':
        ...
        if not preferred_study_setup or preferred_study_setup not in ['2', '5']:
    ...
    df = download_csv()
    existing_idx = find_existing(df, phone, email, connection_type,
                                 country=country, language=language, cohort=cohort,
                                 program=program, topic_module=topic_module,
...
def match_users():
    ...
    if user['connection_type'] == 'find':
        ...
        while len(eligible) >= 1:
            matched_user_idx = eligible.index
            group_id = f"group-{uuid.uuid4()}"
            now_iso = datetime.utcnow().isoformat()
    ...
    if user['matched']:
        ...
        for member in group_members:
            if member['email'] != 'unpaired':
                send_waiting_email(member['email'], member['name'], group_id)
        return jsonify({
            'matched': True,
            'group_id': group_id,
...
def unpair():
    ...
    if user['matched'] and group_id:
        ...
        group_indices = [user_row.index]

    for idx in group_indices:
        df.at[idx, 'email'] = 'unpaired'
        df.at[idx, 'cohort'] = 'unpaired'
        df.at[idx, 'program'] = 'unpaired'
